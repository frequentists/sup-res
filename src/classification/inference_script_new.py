...
class SequenceClassificationDatasetNoLabels(Dataset):
	...
	def collate_fn(self, batch):
		model_inputs = self.tokenizer([i for i in batch], return_tensors="pt", padding=True, truncation=True, max_length=128).to(self.device)
...
if __name__ == "__main__":
	...
	parser.add_argument('--model_name', type=str, default='trained-distilbert-base-uncased-25000/')
	...
	model_names = ["naacl-finetuned-distilbert-base-uncased-10000", "naacl-finetuned-distilbert-base-uncased-20000", "naacl-finetuned-distilbert-base-uncased-50000"]
	...
	for model_name, n_labels in zip(model_names, all_labels):
		...
		with open(os.path.join(model_name, "predictions_devset_" + n_labels + ".json"), "w") as outfile:
		...
		for recall_at in [1,5,10]:
			...
			for i, t in enumerate(true):
				...
				if t in predictions:
					...
					is_correct.append(0)
			print ("n labels test", n_labels, "-- recall @ ", recall_at, np.round(np.mean(is_correct) * 100, 2))
		with open(os.path.join(model_name, "predictions_testset_" + n_labels + ".json"), "w") as outfile:
			json.dump({"true": true, "hits": hits}, outfile)
